#  barplot.sg
#  org http://svn.r-project.org/R/trunk/src/library/graphics/R/barplot.R
#  Modified from the original R library
#  
#
...
barplot.sg <- function(height, ...) UseMethod("barplot.sg")

barplot.sg.default <-
  function(height, width = 1, 
           #efv - Padding space before bars
           space.before = NULL,
           #~efv
           space = NULL,  names.arg = NULL, legend.text = NULL, beside = FALSE, horiz = FALSE,
           ...
           args.legend = NULL, ...)
  {
    ...
    #efv - if space.before is null set to 0, otherwise must be equal to trailing space
    if (is.null(space.before))
      space.before <- 0
    space.before <- space.before * mean(width)
    #efv
    ...
    #efv: this looks like useless code
    #delta <- width / 2 #0.5
    #w.r <- cumsum(space + width) #si space=1 => 2
    #w.m <- w.r - delta - space.before #2 - .5 = 1.5
    #w.l <- w.m - delta - space.before #1.5 - .5 = 1
    ...
    #print(space)
    #print(width)
    w.r <- cumsum(space + width  + space.before)
    w.m <- w.r - delta  
    w.l <- w.m - delta
    #print(w.r)
    #print(w.m)
    ...
    #~efv
    if (horiz) {
      #efv:chk - modificar en horiz despues de probar vert
    ...
    if(plot) { ##-------- Plotting :
      ...
      #efv. chk. que pasa con los beside
      if (beside)
        ...
        for (i in 1L:NC) {
          xyrect(height[1L:NR, i] + offset[i], w.l[i]+space.before,
                 height[ -1,  i] + offset[i], w.r[i]+space.before,
